...
void
TimeStamp2String(uint64_t timestamp, std::string &str) {
    time_t t = timestamp;
...
bool
String2TimeStamp(const std::string &str, uint64_t &timestamp) {
    struct tm stm;
    char *p = strptime(str.c_str(), "%Y-%m-%d %H:%M:%S",&stm);
    if (p) {
...
bool
Timestamp2KeyString(uint64_t timestamp, std::string &key) {
    key.clear();

    char buf[KEY_STRING_LEN];
    snprintf(buf, sizeof(buf), "%llu", timestamp);
    int len = std::string(buf).length();
    int zero_count = KEY_STRING_LEN - len;
...
bool
KeyString2Timestamp(std::string key, uint64_t &timestamp) {
    assert(key.length() == KEY_STRING_LEN);
    while (key[0] == '0') {
        key.erase(0, 1);
    }

    sscanf(key.c_str(), "%llu", &timestamp);
    return true;
}
